function [t_output, iter] = ADMM_small_L(Rhat, Yhat, N, M, K, L, param)
...
for iter = 1 : maxiter
    ...
    % update W, Z, t
    for m = 1 : M
        W_new{m} = Q_m{m}(1:L,1:L) + Lambda_m{m}(1:L,1:L)/mu1(m) - eye(L)/mu1(m);
        L_matrix = Yhat{m} - Q_m{m}(L+1:end,1:L) - Lambda_m{m}(L+1:end,1:L)/mu1(m);
        if norm(L_matrix,'fro') < 1e-6
            Z_new{m} = Yhat{m};
        else
            Z_new{m} = Yhat{m} - max(1 - sqrt(N)/(mu1(m)*norm(L_matrix,'fro')), 0) * L_matrix;
        end
        tmp{m} = eye(N)/mu1(m) - Q_m{m}(L+1:end,L+1:end) - Lambda_m{m}(L+1:end,L+1:end)/mu1(m);
    end
    ...
    for m = 1 : M
        ...
        err_dual1(m) = norm( mu1(m)*[vec(Big_diff{m}(1:L,1:L)); vec(Big_diff{m}(L+1:end,1:L)); toeplitz_approx(Big_diff{m}(L+1:end,L+1:end))],'fro' );
        tol_prim1(m) = sqrt((N+L)*(N+L))*tol_abs + tol_rel*max(norm(Big_new{m},'fro'), norm(Q_m{m},'fro'));

        dual_var_adj(m) = norm([vec(Lambda_m{m}(1:L,1:L)); vec(Lambda_m{m}(L+1:end,1:L)); toeplitz_approx(Lambda_m{m}(L+1:end,L+1:end))],'fro');
        tol_dual1(m) = sqrt((L^2+N*L+2*N-1)*1)*tol_abs + tol_rel*dual_var_adj(m);
    ...
    if verbose
        fprintf('%4g | %.3d %.3d | %.3d %.3d | %.2d %.2d\n',...
            iter,err_prim2,tol_prim2,err_dual2,tol_dual2,etime(clock,T1),etime(clock,T0));
    end
